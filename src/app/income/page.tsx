      ...
      <div className="flex max-w-[1280px] mx-auto">
        ...
        <main className="flex-1 p-6 space-y-4">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-[#0f2040]">รายได้และเอกสาร</h1>
          ...
          <div className="grid md:grid-cols-3 gap-3 text-sm">
            <div className="card p-4"><div className="text-xs text-slate-500">ประมาณการ (Estimated)</div><div className="text-xl font-bold text-amber-600">฿ 7,760</div><div className="text-[11px] text-slate-500">ยังไม่ถือเป็นยอดอนุมัติ</div></div>
            ...
            <div className="card p-4"><div className="text-xs text-slate-500">จ่ายแล้ว (Paid)</div><div className="text-xl font-bold text-[#0f2040]">฿ 14,550</div></div>
          </div>
          <div className="card p-4 overflow-auto">
              ...
              <tbody className="divide-y">
                {rows.map(r=>(
                  <tr key={r.id} className="hover:bg-slate-50">
                    ...
                    <td className="p-2"><span className={`px-2 py-1 rounded-full text-[11px] ${r.status==='ESTIMATED'?'bg-amber-100 text-amber-800': r.status==='PAID'?'bg-[#0f2040] text-white': r.status==='REVERSED'?'bg-red-100 text-red-700':'bg-emerald-100 text-emerald-800'}`}>{r.status}</span></td>
            ...
            <div className="mt-3 flex gap-2 text-xs">
              ...
              <button className="px-3 py-1.5 rounded-full bg-[#0f2040] text-white">ตรวจสอบเอกสาร (QR)</button>
            </div>
          </div>
        </main>
      </div>
