      ...
      <div className="flex max-w-[1280px] mx-auto">
        ...
        <main className="flex-1 p-6 space-y-4">
          <div className="flex items-center gap-3">
            <h1 className="text-xl font-bold text-[#0f2040]">ความก้าวหน้า ⬆</h1>
            <span className="badge-demo">เส้นทางตำแหน่ง</span>
          </div>
          <div className="card p-5">
            <div className="text-sm font-semibold text-[#0f2040]">Progress ไปตำแหน่งถัดไป</div>
            <div className="mt-4 space-y-3">
              {steps.map((s,i)=>(
                <div key={s.name} className={`p-4 rounded-xl border flex items-center gap-4 ${s.have?'bg-emerald-50 border-emerald-200':'bg-white'}`}>
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${s.have?'bg-emerald-600 text-white':'bg-[#0f2040] text-white'}`}>{i+1}</div>
